...
def build_samples_hour(out_dir, df, v):
    ...
    for _, row in df.iterrows():
        ...
        for segment_features, label, participant_id, breed, day, hour, pos_value_count, mean, median, missingness_percentage in hourly_segments:
            new_row = list(segment_features) + [label, participant_id, breed, day, hour, pos_value_count, mean, median, missingness_percentage ]
            data.append(new_row)
    columns = [f'x{i}' for i in range(3600)] + ['label', 'participant_id', 'breed', 'day', 'hour', 'pos_value_count', 'mean', 'median', 'missingness_percentage']
...
def split_features(row):
    ...
    breed = row['breed']
    day = row['date']
    features = row.drop(['label', 'id', 'breed', 'date']).values
    for hour in range(24):
        ...
        hourly_segments.append((segment_features, label, participant_id, breed, day, hour, pos_value_count, mean, median, missingness_percentage))
...
def build_datasets(out_dir):
    ...
    meta_columns = ['label', 'participant_id', 'breed', 'day', 'hour', 'pos_value_count', 'mean', 'median', 'missingness_percentage']
    ...
    df_combined.columns = [f"x{i}" for i in range(len(df_combined.columns)-len(df_meta.columns))] + df_meta.columns.tolist()
    ...
    df_combined_features_xyz = pd.concat([df_accx_features, df_accy_features, df_accz_features], axis=1)
    df_meta = df_x[meta_columns]
    df_combined_xyz = pd.concat([df_combined_features_xyz, df_meta], axis=1)
    df_combined_xyz.columns = [f"x{i}" for i in range(len(df_combined_xyz.columns)-len(df_meta.columns))] + df_meta.columns.tolist()
    df_combined_xyz.to_csv(out_dir / "cleaned_dataset_xyz.csv", index=False)

    df_combined_features_xyz_pulse = pd.concat([df_accx_features, df_accy_features, df_accz_features, df_p_features], axis=1)
    df_meta = df_x[meta_columns]
    df_combined_xyz_pulse = pd.concat([df_combined_features_xyz_pulse, df_meta], axis=1)
    df_combined_xyz_pulse.columns = [f"x{i}" for i in range(len(df_combined_xyz_pulse.columns)-len(df_meta.columns))] + df_meta.columns.tolist()
    ...
    df_combined_features_full = pd.concat([df_accx_features, df_accy_features, df_accz_features,
                                           df_p_features, df_c_features, df_m_features], axis=1)
    df_combined_full = pd.concat([df_combined_features_full, df_meta], axis=1)
    df_combined_full.columns = ([f"accx_{x}" for x in range(len(df_accx_features.columns))] +
...
def main(meta_datafile, input_dir, out_dir):
    ...
    data_dict = {'AccX': [], 'AccY': [], 'AccZ': [], 'MagX': [], 'MagY': [], 'MagZ': [], 'Pulse': [], 'SpO2': [], 'Counts': [], 'Magnitude': []}
    time_dict = {'AccX': [], 'AccY': [], 'AccZ': [], 'MagX': [], 'MagY': [], 'MagZ': [], 'Pulse': [], 'SpO2': [], 'Counts': [], 'Magnitude': []}
    id_dict = {'AccX': [], 'AccY': [], 'AccZ': [], 'MagX': [], 'MagY': [], 'MagZ': [], 'Pulse': [], 'SpO2': [], 'Counts': [], 'Magnitude': []}
    for j, file in enumerate(files):
        ...
        for i, df in enumerate(dfs):
            ...
            stage, breed = get_stage(meta_datafile, df, p_id)
            df = df[vars + ['Timestamp', "Time_dt"]].copy()  # Ensure you are working with a copy
            day = pd.to_datetime(df["Timestamp"].values[0]).strftime('%d/%m/%Y')
            label = f"{p_id}__{stage}__{breed}__{day}"
            df["id"] = label
...
if __name__ == "__main__":
    ...
    meta_datafile = Path(r"C:\Brooke Study Data\meta.csv")
